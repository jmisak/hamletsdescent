...
Features:
  ...
  - Player character uses the same sprites from assets/adventurer (scaled slightly larger).
  - Act I music: "Onloose.mp3".
  - After Act I (after 5 screen scrolls), transition to Level 2 with crow enemies and quotes.
...
LEVEL_WIDTH = 10 * SCREEN_WIDTH  # Total level width for scrolling
...
PLAYER_SCALE = SPRITE_SCALE * 1.1   # Make player slightly larger
GHOST_SCALE  = 0.3                  # Shrink ghosts (based on 204x341 frames)
...
def load_parallax_layers():
    """
    Loads two parallax layers from background files.
    For Level 2, we can reuse existing background images.
    """
    layers = []
    ...
    for i, file in enumerate(filenames):
        if os.path.exists(file):
            ...
            # Scale so height equals SCREEN_HEIGHT
            ...
            # Create a surface that spans LEVEL_WIDTH
            ...
            # Fallback color
            layer_surface = pygame.Surface((LEVEL_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
            layer_surface.fill((100 + i*50, 100, 100, 150))
            layers.append((layer_surface, factors[i]))
    return layers
...
class GhostEnemy(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, speed):
        ...
        if os.path.exists(ghost_sheet_path):
            ...
            for row in range(3):
                ...
                for col in range(5):
                    ...
                    scaled_frame = pygame.transform.scale(frame, (int(frame_width * GHOST_SCALE),
        ...
        self.world_x = x
        self.world_y = y - self.rect.height  # align bottom with y
...
def main_level2(screen, player, score, clock):
    """
    Level 2: Original level with crow enemies and quotes.
    This loop uses parallax layers and spawns crow enemies.
    """
    ...
    while running:
        ...
        pygame.display.flip()
        if time.time() - level2_start_time > 30:
            running = False
...
# ---------------------------
def main():
    global player, score, PIXEL_FONT, level_obj, level_start_time, clock
    pygame.init()
    pygame.font.init()
    try:
        pygame.mixer.init()
    ...
    fixed_player_screen_x = 100
    transition_x = level_start_x + 5 * SCREEN_WIDTH  # After 5 screen-width scrolls, transition to Level 2
