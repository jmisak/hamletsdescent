...
This version includes:
  ...
  - A static background for each level (each spanning 10 screen widths):
      Level 0 uses bg_layer3.png,
      Level 1 uses bg_layer1.png,
      Level 2 uses bg_layer2.png (then cycles).
...
def load_level_background(level_index):
    """
    Load and tile the background for the level.
    Level 0: bg_layer3.png, Level 1: bg_layer1.png, Level 2: bg_layer2.png (then cycles).
    The background is scaled to SCREEN_HEIGHT and tiled to LEVEL_WIDTH.
    """
    mod = level_index % 3
    if mod == 0:
        bg_file = "bg_layer_1.png"
    elif mod == 1:
        bg_file = "bg_layer_2.png"
    else:
        bg_file = "bg_layer_3.png"
    if os.path.exists(bg_file):
        bg_image = pygame.image.load(bg_file).convert_alpha()
    else:
        bg_image = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT))
        bg_image.fill((100, 100, 100))
    scale_factor = SCREEN_HEIGHT / bg_image.get_height()
    new_width = int(bg_image.get_width() * scale_factor)
    scaled_bg = pygame.transform.scale(bg_image, (new_width, SCREEN_HEIGHT))
    level_bg = pygame.Surface((LEVEL_WIDTH, SCREEN_HEIGHT))
...
class Mentor(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        if os.path.exists(mentor_image_path):
            self.image = pygame.image.load(mentor_image_path).convert_alpha()
            # Use the full image (assumed to be cropped to one sprite)
            self.image = pygame.transform.scale(self.image, 
                                                  (int(self.image.get_width()*SPRITE_SCALE), 
        ...
        self.world_x = x
        # Lower her further by adding an offset (adjust as desired)
        self.world_y = SCREEN_HEIGHT - self.rect.height + 50
...
def show_narrative(screen, mentor):
    ...
    It remains visible for 4 seconds or until a key is pressed.
    """
...
def main():
    ...
    level_start_x = 0
    level_bg = load_level_background(current_level_index)
    ...
    while True:
        ...
        if not mentor_spawned and player.world_x >= SCREEN_WIDTH:
            ...
            ground_height = SCREEN_HEIGHT // 4
            mentor_y = SCREEN_HEIGHT - ground_height  # Align with ground
            mentor = Mentor(mentor_x, mentor_y)
            mentor_spawned = True
        ...
        if not chest_spawned and player.world_x >= 4 * SCREEN_WIDTH:
            ...
            ground_height = SCREEN_HEIGHT // 4
            chest_y = SCREEN_HEIGHT - ground_height - 50  # Adjust to place chest on the ground
        ...
        # Draw scene
        screen.fill(BLACK)
        screen.blit(level_bg, (-camera_x, 0))
