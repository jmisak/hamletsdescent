...
def show_opening_scene(screen):
    ...
    Displays the opening scene:
      ...
      - Typewriter effect text appearing word by word.
    ...
    # Play tomb music
    tomb_music_path = "tomb_music.mp3"  # Change extension if needed.
    try:
    ...
    while not opening_done:
        ...
        # Add next word if delay has passed
        ...
        # If all words have been displayed, wait 3 seconds then prompt to press X
        if word_index >= len(words):
            # Ensure text stays on screen for 3 seconds after complete
            ...
            while waiting_for_key:
                ...
                # Blit background and text again
                screen.blit(bg, (0, 0))
                # Redraw the complete typewriter text
                lines = textwrap.wrap(displayed_text, width=70)
                y_offset = 50
                for line in lines:
                    text_surface = font.render(line, True, text_color)
    ...
    # Stop the tomb music and return
...
def main():
    ...
    # First, show the loadscreen.
    show_loadscreen(screen)
    # Then, show the opening scene.
    show_opening_scene(screen)
    
    # Now load main game music
