...
class KnowledgeScroll(pygame.sprite.Sprite):
    """Collectible scrolls that teach about Hamlet"""
    def __init__(self, x, y, quote_data):
        super().__init__()
        self.quote_data = quote_data
        self.image = pygame.Surface((50, 60), pygame.SRCALPHA)
        
        # Draw scroll visual
        pygame.draw.rect(self.image, (220, 200, 150), (5, 0, 40, 50))
        pygame.draw.rect(self.image, (180, 160, 110), (5, 0, 40, 5))
        pygame.draw.rect(self.image, (180, 160, 110), (5, 45, 40, 5))
        # Draw text lines
        for i in range(3):
            pygame.draw.line(self.image, (100, 80, 60), (10, 15 + i*10), (40, 15 + i*10), 2)
    ...
    def update(self, dt):
        self.bob += dt * 2
        self.rect.y = self.rect.y - 5 + math.sin(self.bob) * 5

def show_quote_popup(screen, quote_data, clock):
    """Display educational quote when scroll is collected"""
    overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
    overlay.fill((0, 0, 0, 180))
    
    # Create quote box
    box_width = SCREEN_WIDTH - 400
    box_height = 300
    box = pygame.Surface((box_width, box_height), pygame.SRCALPHA)
    pygame.draw.rect(box, (40, 40, 60), (0, 0, box_width, box_height))
    pygame.draw.rect(box, (220, 200, 150), (0, 0, box_width, box_height), 5)
    
    # Render quote
    quote_lines = textwrap.wrap(f'"{quote_data["quote"]}"', width=60)
    y = 50
    for line in quote_lines:
        text = render_gradient_text(line, PIXEL_FONT, (255, 230, 180), WHITE)
        box.blit(text, (50, y))
        y += 40
        
    # Context
    context_lines = textwrap.wrap(f"Context: {quote_data['context']}", width=70)
    y += 20
    for line in context_lines:
        text = render_gradient_text(line, PIXEL_FONT, (200, 200, 200), (150, 150, 150))
        box.blit(text, (50, y))
        y += 30
        
    # Instructions
    inst = render_gradient_text("Press SPACE to continue", PIXEL_FONT, YELLOW, WHITE)
    box.blit(inst, (box_width//2 - inst.get_width()//2, box_height - 50))
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                if time.time() - start_time > 0.5:  # Prevent accidental skip
                    return
        ...
        screen.blit(box, (200, SCREEN_HEIGHT//2 - box_height//2))
        pygame.display.flip()
        clock.tick(FPS)

def show_trivia_challenge(screen, trivia_data, clock):
    """Educational quiz about Hamlet"""
    overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA)
    overlay.fill((0, 0, 0, 200))
    
    box_width = SCREEN_WIDTH - 300
    box_height = 500
    selected = 0
    answered = False
    correct = False
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN and not answered:
                if event.key == pygame.K_UP:
                    ...
                    answered = True
                    correct = (selected == trivia_data["correct"])
            elif event.type == pygame.KEYDOWN and answered:
        ...
        box = pygame.Surface((box_width, box_height), pygame.SRCALPHA)
        pygame.draw.rect(box, (40, 40, 60), (0, 0, box_width, box_height))
        pygame.draw.rect(box, (100, 100, 150), (0, 0, box_width, box_height), 5)
        
        # Title
        title = render_gradient_text("HAMLET KNOWLEDGE CHALLENGE", PIXEL_FONT, YELLOW, WHITE)
        box.blit(title, (box_width//2 - title.get_width()//2, 20))
        
        # Question
        q_lines = textwrap.wrap(trivia_data["question"], width=60)
        y = 80
        for line in q_lines:
            text = render_gradient_text(line, PIXEL_FONT, WHITE, (200, 200, 200))
            box.blit(text, (50, y))
            y += 35
        ...
        for i, option in enumerate(trivia_data["options"]):
            color = WHITE
            if not answered and i == selected:
                color = YELLOW
            elif answered:
                if i == trivia_data["correct"]:
                    color = GREEN
                elif i == selected and not correct:
                    color = RED
                    
            prefix = "→ " if i == selected and not answered else "  "
            text = render_gradient_text(f"{prefix}{option}", PIXEL_FONT, color, color)
            box.blit(text, (100, y))
            y += 50
        ...
        if answered:
            if correct:
                result = render_gradient_text("Correct! +50 bonus points!", PIXEL_FONT, GREEN, WHITE)
                box.blit(result, (box_width//2 - result.get_width()//2, 400))
            else:
                result = render_gradient_text("Incorrect. No bonus this time.", PIXEL_FONT, RED, WHITE)
                box.blit(result, (box_width//2 - result.get_width()//2, 400))
                
            # Explanation
            exp_lines = textwrap.wrap(trivia_data["explanation"], width=65)
            y = 430
            ...
            inst = render_gradient_text("Press SPACE to continue", PIXEL_FONT, YELLOW, WHITE)
            box.blit(inst, (box_width//2 - inst.get_width()//2, box_height - 30))
        else:
            inst = render_gradient_text("Use ↑↓ to select, ENTER to answer", PIXEL_FONT, (200, 200, 200), WHITE)
            box.blit(inst, (box_width//2 - inst.get_width()//2, box_height - 30))
            
        screen.blit(box, (150, SCREEN_HEIGHT//2 - box_height//2))
        pygame.display.flip()
        clock.tick(FPS)
...
class Player(pygame.sprite.Sprite):
    def __init__(self,x,y):
        ...
        self.rect=self.image.get_rect(midbottom=(x,y-50))  # Adjusted to prevent below screen
...
def main_level2(screen, player, score, clock, adaptive):
    """Enhanced Act 2 with powerups, platforms, and educational content"""
    ...
    for i in range(3):
        x = 600 + i * 800
        y = SCREEN_HEIGHT - 400
        scrolls.add(KnowledgeScroll(x, y, hamlet_quotes[i]))
    
    fixed_x = 100
    start = time.time()
    current_quote_idx = 3  # Start with a different quote
    quote_timer = 0
    ...
    while time.time() - start < 60:
        ...
        # Knowledge scroll collection
        scroll_hits = pygame.sprite.spritecollide(player, scrolls, True)
        for scroll in scroll_hits:
            scrolls_collected += 1
            show_quote_popup(screen, scroll.quote_data, clock)
            score += 25  # Bonus for learning
            
            # Achievement check
            if scrolls_collected >= 3 and not achievements["scholar"]:
                achievements["scholar"] = True
                
        # Show trivia challenge halfway through
        if not trivia_shown and time.time() - start > 30:
            trivia_shown = True
            trivia_idx = random.randint(0, len(hamlet_trivia) - 1)
            if show_trivia_challenge(screen, hamlet_trivia[trivia_idx], clock):
                score += 50  # Bonus for correct answer
                
        # Update quote display
        quote_timer += dt
        if quote_timer > 10:  # Change quote every 10 seconds
            quote_timer = 0
            current_quote_idx = (current_quote_idx + 1) % len(hamlet_quotes)
        ...
        # Current Hamlet quote (educational element)
        quote_data = hamlet_quotes[current_quote_idx]
        quote_text = f'"{quote_data["quote"]}"'
        qs = render_gradient_text(quote_text, PIXEL_FONT, (255, 230, 180), WHITE)
        qr = qs.get_rect(center=(SCREEN_WIDTH/2, 100))
        screen.blit(qs, qr)
        
        # Scroll indicator
        if scrolls:
            scroll_text = render_gradient_text(f"Knowledge Scrolls: {scrolls_collected}/3", PIXEL_FONT, (220, 200, 150), WHITE)
            screen.blit(scroll_text, (SCREEN_WIDTH - 300, 60))
...
def main():
    ...
    while running:
        ...
        if player.rect.x >= transition_x:
            # Show trivia before transitioning
            trivia_idx = random.randint(0, len(hamlet_trivia) - 1)
            bonus = show_trivia_challenge(screen, hamlet_trivia[trivia_idx], clock)
            if bonus:
                score += 50
            ...
            if victory:
                ...
                # Special message for scholars
                if scrolls_collected >= 5:
                    achievements["soliloquy_master"] = True
                    scholar_msg = "True Scholar of Shakespeare!"
                    scholar_text = render_gradient_text(scholar_msg, PIXEL_FONT, PURPLE, WHITE)
                    screen.blit(scholar_text, scholar_text.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2 - 100)))
            else:
            ...
            # Educational summary
            edu_text = render_gradient_text(f"Hamlet Knowledge Gained: {scrolls_collected} scrolls", PIXEL_FONT, (220, 200, 150), WHITE)
            screen.blit(edu_text, (SCREEN_WIDTH//2 - 250, y + 20))
        ...
        for scroll in scrolls:
            ...
            screen.blit(scroll.image, r)
            # Draw glow effect
            pygame.draw.circle(screen, (255, 230, 180, 50), (r.centerx, r.centery), 30, 3)
        ...
        # Educational quote display
        quote_data = hamlet_quotes[quote_idx]
        quote_lines = textwrap.wrap(f'"{quote_data["quote"]}"', width=80)
        y = SCREEN_HEIGHT - 120
        for line in quote_lines:
            quote_surf = render_gradient_text(line, PIXEL_FONT, (255, 230, 180), (255, 255, 200))
            quote_rect = quote_surf.get_rect(center=(SCREEN_WIDTH//2, y))
            # Background for readability
            bg_rect = quote_rect.inflate(20, 10)
            pygame.draw.rect(screen, (0, 0, 0, 180), bg_rect)
            screen.blit(quote_surf, quote_rect)
            y += 30
            
        # Scroll counter
        if scrolls_collected > 0 or len(scrolls) > 0:
            scroll_counter = render_gradient_text(f"Knowledge: {scrolls_collected} scrolls", PIXEL_FONT, (220, 200, 150), WHITE)
            screen.blit(scroll_counter, (SCREEN_WIDTH - 250, 60))
