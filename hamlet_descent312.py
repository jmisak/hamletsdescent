...
# Set both gradient colors to white for white text
TAN_TOP = (255, 255, 255)
...
class Enemy(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, speed):
        pygame.sprite.Sprite.__init__(self)
        # Directly load the crow image from the assets folder:
        crow_path = os.path.join("assets", "crow_fly.png")
        if os.path.exists(crow_path):
            try:
                image = pygame.image.load(crow_path).convert_alpha()
                self.frames = [image]
            except Exception as e:
